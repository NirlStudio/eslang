...
function runTest (args) {
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    if (typeof file === 'string') { // load test suites
      $void.loadData(space, PWD, DIR, file)
    } else {
...
function prepareTestSpace () {
  var appSpace = $void.createAppSpace(PWD + '/.')
  var test = $void.importModule(appSpace, PWD, DIR, 'test')
  Object.assign(appSpace.app, test)
...
module.exports = function () {
  ...
  switch (command) {
    ...
    default:
      return command ? $void.$run(command, args, PWD) : shell(proc, args)
  }
}
