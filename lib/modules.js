...
module.exports = function ($void) {
  ...
  modules.register(function (moduleUri, baseUri) {
  ...
  modules.expose = function (moduleUri/* , ... */) {
    for (var i = 0; i < arguments.length; i++) {
      ...
      if (moduleUri && typeof moduleUri === 'string') {
        ...
        warn('modules:expose', 'invalid moduleUri:', moduleUri, 'at', i)
      }
    }
    return exposed
  }

  modules.exposeFrom = function (baseUri, moduleUri/* , ... */) {
    if (!path.isAbsolute(baseUri)) {
      warn('modules:expose-from', 'invalid baseUri:', baseUri)
      return exposed
    }
    for (var i = 1; i < arguments.length; i++) {
      ...
      if (moduleUri && typeof moduleUri === 'string') {
        ...
        warn('modules:expose-from', 'invalid moduleUri:', moduleUri, 'at', i)
      }
    }
    return exposed
  }
  ...
  modules.resolve = function (source, appHome, appDir, userHome) {
    // a package's or scope's name cannot be '@' or empty.
    if (source === '@' || source.startsWith('@/')) {
      warn('modules:resolve', 'invalid scoped package:', source)
      return null
    }
    ...
    if (source.startsWith('@')) {
      ...
      offset = filePath.indexOf('/', filePath.indexOf('/'))
    } else {
    ...
    if (typeof dir !== 'string') {
      ...
      if (!pack.startsWith('@')) { // not giving a scope yet.
        dir = tryToResolveDir('@eslang/' + pack, paths)
      }
      if (typeof dir !== 'string') {
        warn('modules:resolve', 'failed to resolve', source,
    ...
    if (!fs.existsSync(uri)) {
      warn('modules:resolve', 'failed to resolve', source, 'at', uri)
      return null
    }
