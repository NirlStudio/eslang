...
var render = typeof window === 'undefined' ? require('colors/safe') : null
...
module.exports = function ($void, tracing) {
  ...
  function bindTo (method, type, color) {
    ...
    method = !method ? console.log.bind(console, '#' + type)
      : $void.isNativeHost
        ? method.bind(console, render.gray('#' + type))
        : method.bind(console, '#')

    return $void.isNativeHost ? function () {
      var text = formatArgs.apply(null, arguments)
      method(color ? color(text) : text)
      return text
    } : function () {
